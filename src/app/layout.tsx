import type { Metadata } from "next";
import { Container } from '@/components/foundation/components/Container';
import { Footer } from '@/components/foundation/components/Footer';
import { Content } from '@/components/foundation/components/Box';
import StyledComponentsRegistry from '@/lib/registry'

export const metadata: Metadata = {
  title: "WSH 2024",
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="ja">
      <head>
        <meta charSet="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <meta name="robots" content="noindex, nofollow" />
        <link
          href="data:image/x-icon;base64,"
          rel="icon"
          type="image/x-icon"
        />
        <title>WSH 2024</title>
        <link as="image" crossOrigin="anonymous" href="/assets/cyber-toon.svg" rel="preload" />
      </head>
      <body>
        <StyledComponentsRegistry>
          <Container>
            <Content>
              {children}
            </Content>
            <Footer />
          </Container>
        </StyledComponentsRegistry>
      </body>
    </html>
  );
};
