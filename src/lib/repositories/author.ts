...
import type { DeleteAuthorRequestParams } from "@/lib/schema/api/authors/DeleteAuthorRequestParams";
import type { DeleteAuthorResponse } from "@/lib/schema/api/authors/DeleteAuthorResponse";
...
import type { PatchAuthorRequestBody } from "@/lib/schema/api/authors/PatchAuthorRequestBody";
import type { PatchAuthorRequestParams } from "@/lib/schema/api/authors/PatchAuthorRequestParams";
import type { PatchAuthorResponse } from "@/lib/schema/api/authors/PatchAuthorResponse";
import type { PostAuthorRequestBody } from "@/lib/schema/api/authors/PostAuthorRequestBody";
import type { PostAuthorResponse } from "@/lib/schema/api/authors/PostAuthorResponse";
import {
  author,
  book,
  episode,
...
type AuthorRepositoryInterface = {
  create(options: {
    body: PostAuthorRequestBody;
  }): Promise<Result<PostAuthorResponse, HTTPException>>;
  delete(options: {
    params: DeleteAuthorRequestParams;
  ...
  readAll(options: {
    query: GetAuthorListRequestQuery;
    ...
    try {
      await getDatabase().transaction(async (tx) => {
        ...
        for (const book of deleteBookRes) {
          await tx
            .delete(feature)
            .where(eq(feature.bookId, book.bookId))
            .execute();
          await tx
            .delete(ranking)
            .where(eq(ranking.bookId, book.bookId))
            .execute();
          ...
          for (const episode of deleteEpisodeRes) {
            await tx
              .delete(episodePage)
              .where(eq(episodePage.episodeId, episode.episodeId))
              .execute();
          }
        }
      });
